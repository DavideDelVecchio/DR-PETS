...
# Load dataset (reusing state-action transitions)
...
actions = data[:, state_dim:]
targets = torch.cat([states[1:], states[-1:].clone()], dim=0)  # dummy delta target: next state prediction
